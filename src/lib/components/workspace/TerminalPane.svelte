<script lang="ts">
  ...
  const { workspaceId, terminalId }: Props = $props();
  ...
  onMount(async () => {
    ...
    term.onData((data: string) => {
      ...
      void api.terminal.write(terminalId, bytes);
    });
    ...
    observer = new ResizeObserver(() => {
      try {
        fit.fit();
        if (term) {
          void api.terminal.resize(terminalId, term.cols, term.rows);
        }
      } catch {
        // No layout — skip.
      }
    ...
    // this terminal id from a prior mount. Spawn fresh on rejection,
    // each call with its own Channel id.
    try {
      await api.terminal.reattach(terminalId, makeChannel());
    } catch {
      try {
        await api.terminal.spawn(workspaceId, terminalId, makeChannel(), term.cols, term.rows);
      } catch (err) {
        if (term) {
